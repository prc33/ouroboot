...
/* pend[] values remain on wasm's operand stack and therefore have stale
   locals. Consumers reuse its top prefix; control flow flushes it. */
#define WASM_VS_MAX 16
typedef struct WasmVStack {
    int pend[WASM_VS_MAX];
    int depth;
} WasmVStack;
/* Restore an empty operand stack and current locals. */
...
/* Bind inputs a/b and result dst. Return the reusable operand-stack prefix. */
...
/* Leave a new value on the operand stack. */
static void vs_def(WasmVStack *vs, int local)
{
    vs->pend[vs->depth++] = local;
}
/* Function-wide emitter state. */
                           ...
                           int emit_dispatch)
{
    ...
    int vs_n = 0;
    /* Bind operands before emitting bytes, then define the result. */
#define VS_NO      (-1)
...
static int wasm_scope_depth(WasmScope *scope, int n, int type, int target)
{
    int i;
    for (i = n - 1; i >= 0; i--)
        if (scope[i].type == type && scope[i].target == target)
            return n - 1 - i;
    return -1;
}
...
static void wasm_emit_function_body(WasmBuf *code, WasmFuncIR *f, TCCState *s1)
{
    ...
    int use_frame = 0;
    int local_pc, local_fp, local_cmp, local_i0, local_f0, local_tmp64;
    ...
    direct_type = tcc_malloc(f->nb_ops * sizeof(*direct_type));
    direct_local = tcc_malloc(f->nb_ops * sizeof(*direct_local));
    ...
    for (i = 0; i < f->nb_ops; ++i) {
        ...
        for (j = 0; j < f->nb_ops && safe; ++j) {
            ...
            if ((p->flags & 0xff) == WASM_ADDR_FP && p->imm == off) {
                int ptype = (p->kind == WASM_OP_LOAD_I32 || p->kind == WASM_OP_STORE_I32)
                          ? WASM_VAL_I32
                          : (p->kind == WASM_OP_LOAD_I64 || p->kind == WASM_OP_STORE_I64)
                          ? WASM_VAL_I64 : 0;
                if (ptype != type)
                    safe = 0;
            }
        ...
        if (safe) {
            ...
            direct_type[nb_direct - 1] = type;
            if (type == WASM_VAL_I32) nb_direct_i32++;
            else nb_direct_i64++;
        }
    }
    for (i = 0; i < f->nb_params; ++i)
    ...
    wb_uleb(&body, 3 + !!nb_direct_i32 + !!nb_direct_i64);
    ...
    if (nb_direct_i32)
        wb_uleb(&body, nb_direct_i32), wb_u8(&body, 0x7f);
    if (nb_direct_i64)
        wb_uleb(&body, nb_direct_i64), wb_u8(&body, 0x7e);
    local_pc = f->nb_params;
    local_fp = local_pc + 1;
    local_cmp = local_fp + 1;
    local_i0 = local_cmp + 2;
    local_f0 = local_i0 + WASM_NB_I32_REGS;
    local_tmp64 = local_f0 + WASM_NB_F64_REGS;
    {
        int next_i32 = local_tmp64 + WASM_NB_I64_REGS;
        int next_i64 = next_i32 + nb_direct_i32;
        for (i = 0; i < nb_direct; ++i)
            direct_local[i] = direct_type[i] == WASM_VAL_I32 ? next_i32++ : next_i64++;
    }
    for (i = 0; i < f->nb_ops; ++i) {
        WasmOp *op = &f->ops[i];
        int j;
    ...
    ctx_structured.cmp = local_cmp; ctx_structured.carry = local_cmp + 1;
    ...
    if (f->nb_ops > 0) {
        ...
        tcc_free(is_leader);
        /* Reconstruct structured block/loop/br control flow. */
        ...
        /* Front-end hints distinguish real loops from backward switch edges. */
        ...
        /* TCC emits switch bodies before lookup; restore lookup-first order. */
        ...
        /* TCC emits for-loop increments before bodies; restore source order. */
        for (i = 0; i < f->nb_loops; i++) {
        ...
        /* Backward goto cannot be represented by structured wasm. */
        ...
        /* Nor can a forward branch into a loop. */
        for (b_idx = 0; b_idx < nb_blocks && use_structured; b_idx++) {
            int succs[2], ns2 = 0;
        ...
        if (use_structured) {
        /* Place each forward target's scope, outside any loop it exits. */
        ...
        /* Extend crossing forward scopes until all are properly nested. */
        {
            int changed2;
            do {
                changed2 = 0;
        ...
        /* Emit with an outer function-exit block and nested branch scopes. */
        ...
        for (b_idx = 0; b_idx < nb_blocks; b_idx++) {
             ...
             * targets are INSIDE the loop are opened AFTER the loop scope. */
            {
                ...
                for (t = nb_blocks - 1; t > b_idx; t--)
                    if (blk[t].needs_fwd_scope && blk[t].fwd_scope_open == b_idx
                        && (!blk[b_idx].is_loop_header || t > le)) {
                        wb_u8(&body, 0x02), wb_u8(&body, 0x40);
                        scope[scope_depth].type = 'B';
                        scope[scope_depth++].target = t;
                    }
                /* Open loop scope */
            ...
            for (j = blk[b_idx].start; j < blk[b_idx].end; ++j) {
                ...
                if (is_terminal) {
                    ...
                    if (op->kind == WASM_OP_RET) {
                        int d = wasm_scope_depth(scope, scope_depth, 'B', nb_blocks);
                        wb_u8(&body, 0x0c);
                        wb_uleb(&body, d);
                        ...
                        if (target_bi == b_idx + 1) {
                            ...
                            int d = wasm_scope_depth(scope, scope_depth,
                                target_bi <= b_idx && target_bi < nb_blocks ? 'L' : 'B',
                                target_bi);
                            if (d < 0)
                        ...
                        if (taken_bi == b_idx + 1) {
                            /* Both outcomes are natural fallthrough. */
                        } else {
                            int d;
                            /* Load condition */
                            ...
                            d = wasm_scope_depth(scope, scope_depth,
                                taken_bi <= b_idx && taken_bi < nb_blocks ? 'L' : 'B',
                                taken_bi);
                            if (d < 0)
    ...
    if (s1->nb_errors) {
        ...
        tcc_free(direct_type);
        tcc_free(direct_local);
        tcc_free(body.data);
        return;
    }
