...
ST_FUNC void store(int r, SValue *sv)
{
    ...
    if (fr == VT_LOCAL || (sv->r & VT_SYM)) {
      ...
      /* Absolute-address constant (VT_CONST with no symbol). Upstream
       * errored out here ("implement me"), which broke musl's mallocng
       * among others. Materialise the address into t0 and store
       * through it. t0 is already the backend's scratch register (see
       * gcall_or_jmp), so it is safe to clobber at this point. */
      int64_t addr = sv->c.i;
      int32_t hi = ((int32_t)addr + 0x800) >> 12;
 ...
 * by tccgen.c's TOK_alloca case via expr_eq()) and leaves the result
 * in REG_IRET, matching every other riscv_gen_* helper's convention.
 *
 * "Permanently" is the operative word -- this is why gfunc_epilog
 * can't just trust sp's value at return time anymore; see its
 * s0-based restoration, which is the other half of this. */
ST_FUNC void riscv_gen_alloca(void)
{
    int r = ireg(gv(RC_INT));
    EI(0x13, 0, r, r, 15);              /* addi r, r, 15                */
    EI(0x13, 7, r, r, -16);              /* andi r, r, -16 (round down)  */
    ER(0x33, 0, 2, 2, r, 0x20);          /* sub sp, sp, r                */
...
ST_FUNC void gfunc_epilog(void)
{
    ...
    /* If this function called alloca(), sp is currently lower than the
     * compiler statically expects -- alloca (see riscv_gen_alloca)
     * permanently decrements sp to carve out space that must survive
     * until the function returns, so it can't be undone before this
     ...
     * alloca wasn't called: sp is already s0-v at this point in that
     * case, so this changes nothing for the overwhelming majority of
     * functions.
     *
     * Deliberately scoped to v < 2048 only (the common case, and the
     * only case any of our actual alloca call sites hit). The v>=2048
     * path just below does its OWN incremental sp adjustment assuming
     * sp is untouched since entry; composing a second full restoration
     * on top of that risks a genuinely wrong, hard-to-spot doubled
     * adjustment rather than a clean fix, and this project has already
     * spent real time this session on exactly that class of bug
     * (the read_tp/read_fp register mismatch, the .word-truncation
     * bug). A function combining alloca() with >2KB of its OWN fixed
...
// riscv64 emits machine code directly and never needs to distinguish a
// loop's repeat edge from any other jump to an already-known address --
// see this hint's own comment in tcc.h.
