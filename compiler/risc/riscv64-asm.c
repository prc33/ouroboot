...
static void asm_ternary_opcode(TCCState *s1, int token)
{
    ...
    case TOK_ASM_fsgnj_s:
        ...
        return;
    case TOK_ASM_fsgnjn_d:
        asm_emit_f(token, 0x53 | (4 << 27) | (1 << 25) | (1 << 12), ops, ops + 1, ops + 2);
        return;
    case TOK_ASM_fsgnjn_s:
        asm_emit_f(token, 0x53 | (4 << 27) | (0 << 25) | (1 << 12), ops, ops + 1, ops + 2);
        return;
    case TOK_ASM_fsgnjx_d:
        asm_emit_f(token, 0x53 | (4 << 27) | (1 << 25) | (2 << 12), ops, ops + 1, ops + 2);
        return;
    case TOK_ASM_fsgnjx_s:
        asm_emit_f(token, 0x53 | (4 << 27) | (0 << 25) | (2 << 12), ops, ops + 1, ops + 2);
        return;
    case TOK_ASM_fmax_d:
...
static void asm_quaternary_opcode(TCCState *s1, int token)
{
    ...
    if (!strncmp(name, "fmadd.", 6)) opcode = 0x43;
    else if (!strncmp(name, "fmsub.", 6)) opcode = 0x47;
    else if (!strncmp(name, "fnmsub.", 7)) opcode = 0x4b;
    else if (!strncmp(name, "fnmadd.", 7)) opcode = 0x4f;
    else { expect("fused floating-point instruction"); return; }
    asm_emit_fq(token, opcode | (strstr(name, ".d") != NULL) << 25
                | 7 << 12, ops, ops + 1, ops + 2, ops + 3);
}
...
ST_FUNC void asm_opcode(TCCState *s1, int token)
{
    ...
    case TOK_ASM_sfence_vma:
        {
            Operand ops[2];
            parse_operands(s1, ops, 2);
            asm_emit_opcode(0x12000073 | ENCODE_RS1(ops[0].reg)
                             | ENCODE_RS2(ops[1].reg));
        }
        return;
    ...
    case TOK_ASM_fmsub_d:
    case TOK_ASM_fmsub_s:
    case TOK_ASM_fnmsub_d:
    case TOK_ASM_fnmsub_s:
    case TOK_ASM_fnmadd_d:
    case TOK_ASM_fnmadd_s:
...
static int asm_parse_csrvar(int t)
{
    ...
    case TOK_ASM_time:
        return 0xc01;
    case TOK_ASM_sstatus:
        return 0x100;
    case TOK_ASM_sie:
        return 0x104;
    case TOK_ASM_stvec:
        return 0x105;
    case TOK_ASM_scounteren:
        return 0x106;
    case TOK_ASM_sscratch:
        return 0x140;
    case TOK_ASM_sepc:
        return 0x141;
    case TOK_ASM_scause:
