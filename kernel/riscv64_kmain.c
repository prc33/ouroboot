...
void kmain(unsigned long hartid, unsigned long dtb) {
	...
	syscall_init();
	process_init();
	struct ramfs_dynamic_file *init_elf = required_initrd_file("interactive_test");
	struct process *init = init_elf ? process_create_from_elf(init_elf->data, init_elf->size, "init") : 0;
	if (!init) {
