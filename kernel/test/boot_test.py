...
def boot_and_capture(kernel_path, timeout, mem_mb, extra_qemu_args=None):
	cmd = [
		"timeout", str(timeout),
		"qemu-system-i386",
		"-kernel", kernel_path,
...
def run(kernel_path, must_contain, must_not_contain, timeout, mem_mb, extra_qemu_args=None):
	print(f"--- booting {kernel_path} (timeout={timeout}s, mem={mem_mb}MB) ---")
	output, qemu_exit = boot_and_capture(kernel_path, timeout, mem_mb, extra_qemu_args)
...
def main():
	...
	ap.add_argument("--qemu-arg", action="append", default=[], dest="extra_qemu_args",
	                 help="extra raw arg to pass to qemu-system-i386 (repeatable)")
	args = ap.parse_args()

	rc = run(args.kernel, args.must_contain, args.must_not_contain,
	         args.timeout, args.mem_mb, args.extra_qemu_args)
	sys.exit(rc)
