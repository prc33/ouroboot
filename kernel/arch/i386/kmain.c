...
/* Set once, early in kmain(), from the real multiboot info -- read
 * later by checkpoint 3's own initrd load, which runs deep inside a
 * syscall-exit hook chain with no direct access to mb_info_addr
 * anymore. 0/0 if QEMU wasn't given a `-initrd` at all (checkpoint 3
 * reports this as fatal, same as riscv64's own "an explicit initrd
 * tar is required" -- see arch/risc/riscv64_kmain.c). */
static unsigned int g_initrd_base = 0;
static unsigned int g_initrd_size = 0;
...
/* checkpoint 19: real process table + a real initrd-loaded BusyBox --
 * the first time i386 exercises arch/i386/process.c's per-address-
 * space paging or syscall_posix.c's fork/exec/wait4-style syscalls
 * against a real multi-process workload, not just the original
 * single-shot P4/P5 ring3 demo above. Deliberately still reached via
 * the same hook-chaining mechanism as checkpoint 1->2
 * (checkpoint1_to_checkpoint2()'s own comment) rather than
 * restructuring i386's boot into a product-vs-checkpoint split the
 * way riscv64's own kmain() has -- see
 * docs/kernel-arch-split-plan.md for why that's left as a followup,
 * not a blocker, for i386 self-hosting itself. */
static void run_process_table_test(void) {
	/* Not every boot supplies one -- the original P4/P5 test (this
	 * kernel's own `make test`) never did, and still shouldn't have to
	 * just to keep exercising the single-shot ring3/ELF-loader path
	 * checkpoints 1-2 cover. A missing initrd here is a deliberate
	 * "checkpoint 3 doesn't apply to this boot", not a fatal error --
	 * `make test-initrd` (docs/kernel-arch-split-plan.md) is what
	 * supplies one and exercises this. */
	if (!g_initrd_size) {
		kprintf("no initrd supplied -- skipping checkpoint 3 (process table + BusyBox)\n");
		kprintf("halting.\n");
		arch_halt_forever();
	}
	unsigned int initrd_files = tar_load_initrd((const unsigned char *)(unsigned long)g_initrd_base, g_initrd_size);
	kprintf("initrd: %u file(s) loaded from tar at %p\n", initrd_files, (void *)(unsigned long)g_initrd_base);
	if (!initrd_files) {
	...
	struct process *init = process_create_from_elf_argv(busybox_elf->data, busybox_elf->size, ash_argv, 2);
	if (!init) {
		kprintf("FATAL: process_create_from_elf_argv failed\n");
		arch_halt_forever();
	}
	kprintf("P6 checkpoint: real process table + initrd BusyBox starting\n");
	process_run(init);
	/* never reached: every real process's exit eventually reaches
	 * sched/process.c's own process_halt() */
}

/* Checkpoint 2's own conclusion -- see checkpoint1_to_checkpoint2()'s
 * comment above for why this lives here instead of syscall.c. Chains
 * into the real process-table/initrd/BusyBox test above rather than
 * halting -- see that function's own comment. */
static void conclude_checkpoint2(void) {
	kprintf("ring3 test OK\n");
	kprintf("P5 checkpoint 2 OK\n");
	run_process_table_test();
}
...
void kmain(unsigned int magic, unsigned int mb_info_addr) {
	...
	/* checkpoint 19: the initrd QEMU's `-initrd <file>` loads as a
	 * Multiboot module -- flags bit 3 (0x8) means mods_count/mods_addr
	 * are valid. Only the first module is used; this kernel never asks
	 * for more than one. */
	if ((mbi->flags & 0x8) && mbi->mods_count >= 1) {
		struct multiboot_module *mod = (struct multiboot_module *)(unsigned long)mbi->mods_addr;
		g_initrd_base = mod->mod_start;
		g_initrd_size = mod->mod_end - mod->mod_start;
		kprintf("multiboot: initrd module at %p, %u bytes\n",
			(void *)(unsigned long)g_initrd_base, g_initrd_size);
	}
	...
	if (g_initrd_size)
		pmm_reserve_range(g_initrd_base, g_initrd_base + g_initrd_size);
